...
- `external build active`: agentic loop、继续条件、终止条件
...
| **prompt_too_long** | 第 1178 行 | 413 错误进入 overflow 边界；当前仓库里 `reactiveCompact` 为 stub，因此不应文档化为稳定 live 恢复路径 |
...
### 3. Prompt-Too-Long 边界（第 1088-1186 行）
当遇到 413 错误时，`query.ts` 保留了两段恢复性设计分支：
- **Context Collapse Drain**：提交所有已暂存的折叠，释放空间后重试，但依赖 `contextCollapse` feature gate
- **Reactive Compact**：触发即时压缩并重试，但当前 reverse-engineered 仓库里 `reactiveCompact.ts` 是 stub

因此阅读这里时，应该把它理解为“状态机预留了 overflow 恢复形状”，而不是“当前 external build 一定会自动走完这两步”。
