...
description: "深度解析 Claude Code 上下文压缩的主要实现：传统 API 摘要压缩、MicroCompact 局部压缩，以及当前 reverse-engineered 仓库里哪些 PTL/Reactive 分支是真实可用，哪些只是门控或 stub。"
...
<Warning>
这页包含“设计中存在的压缩层”和“当前 reverse-engineered 仓库里真实活跃的压缩层”两部分。尤其是 `reactiveCompact`，在当前仓库里实现文件是 stub，不能当作 external build 的稳定能力。
</Warning>
...
// 当前 reverse-engineered 仓库里 reactiveCompact 实现为 stub，
// 这里保留的是设计上的分支形状，不应直接视为 external active。
...
## PTL 边界：Prompt Too Long

当压缩后仍然超出 token 限制时，query 状态机会进入更保守的 overflow 边界处理。需要区分两件事：

1. **设计上的恢复分支**：`query.ts` 里保留了 `reactiveCompact` 和 collapse drain 相关分支形状。
2. **当前 reverse-engineered 仓库的真实实现**：`src/services/compact/reactiveCompact.ts` 是 stub，`contextCollapse` 又受 feature gate 控制，因此不应写成“当前 external build 一定会自动 reactive compact 并恢复成功”。

更保守的理解是：PTL 在当前仓库里首先是“上下文治理边界”，而不是一个已经完全打通的稳定恢复功能承诺。
...
1. [Token 预算](/docs/context/token-budget)
...
- `external build active`: `microCompact.ts`、`compact.ts`、主要 compact boundary 机制
- `feature-gated`: Session Memory、Snip、部分 collapse 路径
- `stubbed/removed`: `reactiveCompact.ts`
